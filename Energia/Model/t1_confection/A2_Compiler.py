...
# For the production of the parameter files:
Wide_Param_Header = \
...
other_setup_parameters = \
    pd.read_excel('./A2_Extra_Inputs/A-Xtra_Scenarios.xlsx')
...
for n in range(len(other_setup_params_name)):
    other_setup_params.update({other_setup_params_name[n]:
                               other_setup_params_param[n]})
...
this_dict_4_wide = {}
this_dict_4_wide.update({'PARAMETER':[], 'Scenario':[], 'TIMESLICE':[],
                         'YEAR':[], 'Value':[]})
for y in range(len(time_range_vector)):
    this_dict_4_wide['PARAMETER'].append('Yearsplit')
    this_dict_4_wide['Scenario'].append(other_setup_params['Main_Scenario'])
    this_dict_4_wide['TIMESLICE'].append(other_setup_params['Timeslice'])
    this_dict_4_wide['YEAR'].append(time_range_vector[y])
    this_dict_4_wide['Value'].append(1)

df_Yearsplit_gen = pd.DataFrame.from_dict(this_dict_4_wide)
df_Yearsplit = pd.concat([df_Yearsplit, df_Yearsplit_gen], ignore_index=True)
...
for g in range(len(groups_list)):
    ...
    for t in range(len(this_df_techs)):
        ...
        # if this_tech == 'BLEND_DSL':
        #     rev_num_appeas = [i for i, x in enumerate(this_df_techs) if x == this_tech]
        #     print(rev_num_appeas)
        #     print('check repeated connections')
        #     sys.exit()
        ...
        for y in range(len(time_range_vector)):
            ...
            # if anytrue:
                ...
                df_OAR_dict['PARAMETER'].append(this_param)
                df_OAR_dict['Scenario'].append(other_setup_params['Main_Scenario'])
                df_OAR_dict['REGION'].append(other_setup_params['Region'])
                df_OAR_dict['TECHNOLOGY'].append(this_tech)
                df_OAR_dict['FUEL'].append(output_fuel)
                df_OAR_dict['MODE_OF_OPERATION'].append(other_setup_params['Mode_of_Operation'])
                df_OAR_dict['YEAR'].append(time_range_vector[y])
                df_OAR_dict['Value'].append(deepcopy(this_proj_df_new.loc[mask, time_range_vector[y]][this_mask_index]))
                # df_OAR = df_OAR.append(df_OAR_dict, ignore_index=True)
                # df_OAR_local = pd.DataFrame.from_dict(df_OAR_dict)
                # df_OAR_ref = deepcopy(df_OAR)
                # df_OAR = pd.concat([df_OAR_ref, df_OAR_local], ignore_index=True)
            ...
            if groups_list[g] != 'Primary':
                ...
                for inp in range(len(input_fuel_list)):
                    ...
                    # if anytrue:
                        this_mask_index = this_proj_df_new.loc[mask , time_range_vector[y]].index.tolist()[0]
                        df_IAR_dict['PARAMETER'].append(this_param)
                        df_IAR_dict['Scenario'].append(other_setup_params['Main_Scenario'])
                        df_IAR_dict['REGION'].append(other_setup_params['Region'])
                        df_IAR_dict['TECHNOLOGY'].append(this_tech)
                        df_IAR_dict['FUEL'].append(this_input_fuel)
                        df_IAR_dict['MODE_OF_OPERATION'].append(other_setup_params['Mode_of_Operation'])
                        df_IAR_dict['YEAR'].append(time_range_vector[y])
                        df_IAR_dict['Value'].append(deepcopy(this_proj_df_new.loc[mask , time_range_vector[y]][this_mask_index]))
                        # df_IAR = df_IAR.append(df_IAR_dict, ignore_index=True)
                        # df_IAR_local = pd.DataFrame.from_dict(df_OAR_dict)
                        # df_IAR_ref = deepcopy(df_IAR)
                        # df_IAR = pd.concat([df_IAR_ref, df_IAR_local], ignore_index=True)
...
df_OAR_gen = pd.DataFrame.from_dict(df_OAR_dict)
df_IAR_gen = pd.DataFrame.from_dict(df_IAR_dict)
...
for n in range(len(Projection_Driver_Vars)):
    ...
    if Projection_Mode_per_Driver[n] == 'Flat after final year':
        Projections_sheet[this_col_header].interpolate(method ='linear', limit_direction ='forward', inplace = True)
#
demand_headers = ['Demand/Share', 'Fuel/Tech', 'Ref.Cap.BY', 'Ref.OAR.BY',
                  'Ref.km.BY', 'Projection.Mode', 'Projection.Parameter',
                  'Introduced.Unit', 'Target.Unit']
#
list_demand_or_share = Demand_df['Demand/Share'].tolist()
#
'''
The section below to obtain "Fuels_techs_2_dems" mainly applies to transport
'''
list_fuel_or_tech = Demand_df['Fuel/Tech'].tolist()

Fuels_dems = [i for i in range(len(list_fuel_or_tech))
              if 'DEMTRN' in list_fuel_or_tech[i]] 
...
for m in range(len(list_demand_or_share)):
    ...
    if 'GDP coupling' in list_projection_mode[m]:
        ...
        for y in range(len(Projections_sheet['Year'].tolist())):
            ...
            if 'passenger' in list_projection_param[m]:
                demand_trajectory.append(demand_trajectory[-1]*(1 + gr_e_pass*var_gdp))
            if 'freight' in list_projection_param[m]:
                demand_trajectory.append(demand_trajectory[-1]*(1 + gr_e_fre*var_gdp))
            Demand_df.loc[m, this_yr] = round(demand_trajectory[-1]*(this_value_BY/(this_value_BY + other_value_BY)), 4)
        #
    #
    '''
...
for g in range(len(groups_list)):
    ...
    if type(oar) == float or type(oar) == int:
        for d in range(this_group_tech_index):
            if 'DEMTRN' in Demand_df_new.loc[d, 'Fuel/Tech']:  # !!! Hardcoded condition
            ...
            if type(this_unit_introduced) == str:
                ...
                if type(ref_km) == float or type(ref_km) == int:
                    ...
                    for y in range(len(this_km_list_change)):
                        #if (y == 2 or y == 3) and ('BUS' not in this_tech) and ('TAX' not in this_tech) and ('SED' not in this_tech):
                        #    this_km_list.append(round(this_km_list[-1], 4)) # this should be added to a dataframe
                        #    print('!!!', this_tech, y)
                        #else:
                        #    this_km_list.append(round(this_km_list[-1]*(1+this_km_list_change[y]), 4)) # this should be added to a dataframe
                        this_km_list.append(round(this_km_list[-1]*(1+this_km_list_change[y]), 4)) # this should be added to a dataframe
                    #
                    Fleet_Groups_Distance.update({this_tech:this_km_list})
                    # print('    ', this_tech)
                    # if 'TRNFREHEA' in this_tech:
                    # if 'TRNMOT' in this_tech:
                    #     print('check that something is off')
                    #    sys.exit()
                    #
                ...
                #
            #
            this_param_fleet_list = ['TotalAnnualMaxCapacity',
                                     'TotalTechnologyAnnualActivityLowerLimit']
                                     # 'TotalAnnualMinCapacityInvestment']
            if this_param in this_param_fleet_list:
