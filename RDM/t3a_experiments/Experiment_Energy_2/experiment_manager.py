        ...
        for k in range( len( this_var_dict[this_variable] ) ):
            ...
            if this_variable == 'TotalTechnologyAnnualActivity' or this_variable == 'NewCapacity':
                ...
                if this_tech in list( Fleet_Groups_inv.keys() ):
                    ...
                    #
                    if group_tech in ['TRNMIC', 'TRNBUS', 'TRNTAX', 'TRNMOT', 'TRNSED', 'TRNSUV', 'TRNCAM', 'TRNFREHEA', 'TRNFREMED', 'TRNFRELIG', 'TRNCATTRU']:
                        ''' Debug section start
                        '''
                        try:
                            ...
                            print('error here')
                        #
                        if group_tech not in ['TRNMIC', 'TRNSED', 'TRNSUV', 'TRNCAM']:
...
def main_executer(n1, Executed_Scenario, packaged_useful_elements):
    ...
    # if str_scen in ['DDP'] and int(str_fut) in [42, 49, 79, 106, 116, 131, 139, 171, 189]:
    if str_scen in ['DDP']:
        #
...
if __name__ == '__main__':
    ...
    global index_2024
    index_2024 = time_range_vector.index( 2024 )
    #
    global final_year
    ...
    trn_list = ['TRNMIC', 'TRNBUS', 'TRNTAX', 'TRNPASRAI', 'TRNMOT', 'TRNSED',
                'TRNSUV', 'TRNCAM', 'TRNFREHEA', 'TRNFREMED', 'TRNFRELIG',
                'TRNCATTRU', 'TRNFRERAI']
    ...
    for s in range( len( scenario_list ) ):
        ...
        # for f in range( 1, 1+1 ):
            ...
            for u in range( 1, len(experiment_dictionary)+1 ): # u is for the uncertainty number // start at 1 because we are not experimenting the GDP
                ...
                '''
                if f == 2 and scenario_list[s] == 'DDP':
                    print('\n')
                    group_set_range_indices_st = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalAnnualMaxCapacity' ]['t'] ) if x == str( 'TRNTAX' ) ]
                    group_value_list_st = inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalAnnualMaxCapacity' ]['value'][ group_set_range_indices_st[0]:group_set_range_indices_st[-1]+1 ]
                    group_value_list_st = [ float( j ) for j in group_value_list_st ]
                    
                    demand_indices = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'SpecifiedAnnualDemand' ][ 'f' ] ) if x == Fleet_Groups_techs_2_dem['TRNTAX']]
                    ...
                    print(group_value_list_st[-1], '0', demand_list[-1])
                '''
                        ...
                        #
                            ...
                            if 'End' in Math_Type:
                                ...
                                for a_set in range( len( pending_sets ) ):
                                    ...
                                    # if this_set == 'PP_SPV_DG' and scenario_list[s] == 'DDP' and f == 2:
                                    #     print('check again 2')
                                    #     sys.exit()
                        ...
                        # The X type below is manipulated with immediate restitution after adjustment.
                            ...
                            for a_set in range( len( Sets_Involved ) ):
                                ...
                                if Math_Type == 'Adoption_Curve':
                                    ...
                                    #
                                    passpub_range_indices_BAU = [ i for i, x in enumerate( stable_scenarios['BAU'][ 'SpecifiedAnnualDemand' ][ tsti ] ) if x == str( 'DEMTRNPASPUB' ) ]
                                    ...
                                    #
                                    passpriv_range_indices_BAU = [ i for i, x in enumerate( stable_scenarios['BAU'][ 'SpecifiedAnnualDemand' ][ tsti ] ) if x == str( 'DEMTRNPASPRI' ) ]
                                    ...
                                    passpub_range_indices = [ i for i, x in enumerate( stable_scenarios[scenario_list[s]][ 'SpecifiedAnnualDemand' ][ tsti ] ) if x == str( 'DEMTRNPASPUB' ) ]
                                    ...
                                    #
                                    passpriv_range_indices = [ i for i, x in enumerate( stable_scenarios[scenario_list[s]][ 'SpecifiedAnnualDemand' ][ tsti ] ) if x == str( 'DEMTRNPASPRI' ) ]
                                    ...
                                    #
                                    passnomot_range_indices = [ i for i, x in enumerate( stable_scenarios[scenario_list[s]][ 'SpecifiedAnnualDemand' ][ tsti ] ) if x == str( 'DEMTRN_NOMOT' ) ]
                                ...
                                if 'DEMTRN_NOMOT' in this_set:
                                    nonmot_values = deepcopy( new_value_list_rounded )# this retrieves non-motorized
                                    # we must retrieve the value of public transport
                                    passpub_range_indices = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'SpecifiedAnnualDemand' ][ tsti ] ) if x == str( 'DEMTRNPASPUB' ) ]
                                    ...
                                    this_set_range_indices = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'SpecifiedAnnualDemand' ][ tsti ] ) if x == str( 'DEMTRNPASPRI' ) ]
                                ...
                                for a_set in range( len( Sets_Involved ) ):
                                    ...
                                    if this_parameter != 'CapacityFactor' and len(this_set_range_indices) != 0:
                                        ...
                                        if this_parameter == 'OutputActivityRatio' and 'Adjustment OAR' not in X_Cat: # We must adjust the transport group capacities once we obtain new occupancy rates.
                                            ...
                                            train_pass_capacity_indices = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalAnnualMaxCapacity' ][ 't' ] ) if x == str( 'TRNPASRAI' ) ]
                                            ...
                                            #
                                            if this_set == 'TRNSED': # we must adjust motorcycles for demand, as it is not explored for occupancy rate
                                                #
                                                motorcycle_range_indices_max_cap = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalAnnualMaxCapacity' ][ tsti ] ) if x == str( 'TRNMOT' ) ]
                                                ...
                                                #
                                                motorcycle_range_indices_min_cap = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalTechnologyAnnualActivityLowerLimit' ][ tsti ] ) if x == str( 'TRNMOT' ) ]
                            ...
                            for a_set in range( len( Sets_Involved ) ):
                                ...
                                if len(this_set_range_indices) == len(time_range_vector) and preliminary_pass is True:
                                    ...
                                    if Math_Type == 'Adoption_Curve':
                                        ...
                                        if 'Electrification of Private Transport' == X_Cat and group_tech_set == 'TRNSED':
                                    ...
                                    if this_set in [ 'TRBUSHYD', 'TRMBUSHYD', 'TRNFREHEAHYD', 'TRNFREMEDHYD' ]:
                                        ...
                                        if this_set == 'TRNFREHEAHYD':
                                            corresponding_set = 'TRNFREHEAELE'
                                        if this_set == 'TRNFREMEDHYD':
                                            corresponding_set = 'TRNFREMEDELE'
                                            print('HAPPENS???')
                                            sys.exit()
                                        #
                                    ...
                                    exception_list = ['TRNFREHEAELE', 'TRNFREHEAHYD'] # this should happen when hydrogen is modified afterwards
                                    special_list = ['TRNFREHEAHYD']
                                    special_list_dict = {'TRNFREHEAHYD':['TRNFREHEAELE', 'TRNFREHEAHYD']}

                                    if this_set not in exception_list and this_parameter == 'TotalTechnologyAnnualActivityLowerLimit':
                                        rem_sets = [i for i in Fleet_Groups[group_tech_set] if i != this_set]
                                        # Get sum:
                            ...
                            for a_set in range( len( Sets_Involved ) ):
                                ...
                                if ((this_demand_set == 'DEMTRNPASPRI' and dem_adj_private is True) or
                                    ...
                                    if this_demand_set == 'DEMTRNPASPRI':
                                        demand_indices = [ i for i, x in enumerate(inherited_scenarios[scenario_list[s]][f]['SpecifiedAnnualDemand']['f']) if x == str('DEMTRN_NOMOT')]
                                        demand_list = [float(val) for val in inherited_scenarios[scenario_list[s]][f]['SpecifiedAnnualDemand']['value'][demand_indices[0]:demand_indices[-1]+1]]
                                        #
                                        new_value_list = []
                                        for n in range( len( value_list ) ):
                                ...
                                if (this_set in trn_list and this_parameter in ['TotalAnnualMaxCapacity', 'TotalTechnologyAnnualActivityLowerLimit'] ): # THIS CONDITION WILL ALWAYS BE TRUE
                                    ...
                                    if len(this_set_range_indices) != 0:
                                        ...
                                        #
                                        if this_parameter == 'TotalAnnualMaxCapacity' and this_set in ['TRNMIC', 'TRNBUS', 'TRNTAX', 'TRNPASRAI']:
                ...
                if 'Adjustment OAR' in X_Cat: # right after maximum vehicle capacity, for all scenarios, to include the changes in OutputActivityRatio
                    ...
                    if f == 2 and scenario_list[s] == 'DDP':
                        ...
                        if int(group_value_list_st[-1]) == 0:
                    ...
                    for par in range( len( params_to_adjust ) ):
                        for a_set in range( len( Sets_Involved ) ):
                            #
                            this_set = Sets_Involved[ a_set ]
                            if this_set in ['TRNMIC', 'TRNBUS', 'TRNTAX', 'TRNPASRAI']:  # except, pass
                                this_set_range_indices = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ params_to_adjust[par] ][ 't' ] ) if x == this_set]
                                value_list = deepcopy( inherited_scenarios[ scenario_list[s] ][ f ][ params_to_adjust[par] ]['value'][ this_set_range_indices[0]:this_set_range_indices[-1]+1 ] )
                                value_list = [ float( value_list[j] ) for j in range( len( value_list ) ) ]
                                #
                ...
                """
                print('Review changes in lower limit of fossil technologies')
                tech_trn_sedgsl = 'TRNSEDGSL'
                tech_trn_sedele = 'TRNSEDELE'
                tech_trn_sed = 'TRNSED'
                if scenario_list[ s ] == 'DDP':
                    adjust_cap_indices = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalTechnologyAnnualActivityLowerLimit' ][ 't' ] ) if x == tech_trn_sedgsl]
                    adjust_cap_values = deepcopy( inherited_scenarios[ scenario_list[ s ] ][ f ]['TotalTechnologyAnnualActivityLowerLimit']['value'][ adjust_cap_indices[0]:adjust_cap_indices[-1]+1 ] )
                    adjust_cap_values = [ float( adjust_cap_values[j] ) for j in range( len( adjust_cap_values ) ) ]

                    adjust_cap_indices_f0 = [ i for i, x in enumerate( stable_scenarios[ scenario_list[ s ] ][ 'TotalTechnologyAnnualActivityLowerLimit' ][ 't' ] ) if x == tech_trn_sedgsl]
                    adjust_cap_values_f0 = deepcopy( stable_scenarios[ scenario_list[ s ] ]['TotalTechnologyAnnualActivityLowerLimit']['value'][ adjust_cap_indices_f0[0]:adjust_cap_indices_f0[-1]+1 ] )
                    adjust_cap_values_f0 = [ float( adjust_cap_values_f0[j] ) for j in range( len( adjust_cap_values_f0 ) ) ]

                    adjust_cap_indices_ele = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalTechnologyAnnualActivityLowerLimit' ][ 't' ] ) if x == tech_trn_sedele]
                    adjust_cap_values_ele = deepcopy( inherited_scenarios[ scenario_list[ s ] ][ f ]['TotalTechnologyAnnualActivityLowerLimit']['value'][ adjust_cap_indices_ele[0]:adjust_cap_indices_ele[-1]+1 ] )
                    adjust_cap_values_ele = [ float( adjust_cap_values_ele[j] ) for j in range( len( adjust_cap_values_ele ) ) ]
    
                    adjust_cap_indices_ele_f0 = [ i for i, x in enumerate( stable_scenarios[ scenario_list[ s ] ][ 'TotalTechnologyAnnualActivityLowerLimit' ][ 't' ] ) if x == tech_trn_sedele]
                    adjust_cap_values_ele_f0 = deepcopy( stable_scenarios[ scenario_list[ s ] ]['TotalTechnologyAnnualActivityLowerLimit']['value'][ adjust_cap_indices_ele_f0[0]:adjust_cap_indices_ele_f0[-1]+1 ] )
                    adjust_cap_values_ele_f0 = [ float( adjust_cap_values_ele_f0[j] ) for j in range( len( adjust_cap_values_ele_f0 ) ) ]

                    adjust_cap_indices_tot = [ i for i, x in enumerate( inherited_scenarios[ scenario_list[ s ] ][ f ][ 'TotalTechnologyAnnualActivityLowerLimit' ][ 't' ] ) if x == tech_trn_sed]
                    adjust_cap_values_tot = deepcopy( inherited_scenarios[ scenario_list[ s ] ][ f ]['TotalTechnologyAnnualActivityLowerLimit']['value'][ adjust_cap_indices_tot[0]:adjust_cap_indices_tot[-1]+1 ] )
                    adjust_cap_values_tot = [ float( adjust_cap_values_tot[j] ) for j in range( len( adjust_cap_values_tot ) ) ]

                    elec_share = [adjust_cap_values_ele[i]/v for i, v in enumerate(adjust_cap_values_tot)]
                    fossil_share = [adjust_cap_values[i]/v for i, v in enumerate(adjust_cap_values_tot)]

                    print('\n')
                    print(u, X_Cat, fut_id)
                    print('GDL')
                    print(adjust_cap_values)
                    print(adjust_cap_values_f0)
                    print('ELE')
                    print(adjust_cap_values_ele)
                    print(adjust_cap_values_ele_f0)
                    if 'Electrification of Private Transport' == X_Cat:
                        print('ELE_SH')
                        print(electric_sedan_adoption_sh)
                    print(adjust_cap_values_tot)
                    print(elec_share)
                    print(fossil_share)
                    # sys.exit()
                    print('\n')
        ...
        if parallel_or_linear == 'Parallel':
            ...
            for n in range(0,y_ceil):
                ...
                for n2 in range( n_ini , max_iter ):
                    ...
                    fut = all_futures[fut_index - scen*len( all_futures ) ]
                    #
                    # if scenario_list_print[scen] == 'DDP' and int(fut) in [42, 49, 79, 106, 116, 131, 139, 171, 189]:
                    if scenario_list_print[scen] == 'DDP' or scenario_list_print[scen] == 'BAU':
