...
def data_processor( case, Executed_Scenario, unpackaged_useful_elements ):
    ...
    for var in range( len( vars_as_appear ) ):
        ...
        for k in range( len( this_var_dict[this_variable] ) ):
            ...
            #
            if this_combination[2] in ['belleza', 'regulacion_hidrica', 'Turismo_recreacion'] and this_variable == 'AnnualTechnologyEmissionPenaltyByEmission':
                ref_index = combination_list.index( this_combination )
                this_data_row = deepcopy( data_row_list[ ref_index ] ) # this must be updated in a further position of the list
                #
...
#            f.write( 'param ' + this_param + ':=\n' )
            ...
            if len(this_param_keys) == 2 and manipulate_param_keys_2 is True: #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
                ...
                # print(this_param, this_param_keys, 'pay attention!')
    ...
    # g.write('param YearSplit default 1 :=\n;\n')
    # g.write('param OperationalLife default 1 :=\n;\n')
    # g.write('param EmissionsPenalty default 0 :=\n;\n')
    ###
    #
    g.write('#\n' + 'end;\n')
...
def interpolation_non_linear_final(time_list, value_list, new_relative_final_value, finyear):
    ...
    ### print(Initial_Year_of_Uncertainty, finyear)
    ### print(len(fraction_time_list), fraction_time_list)
    ### print(len(fraction_value_list), fraction_value_list)
    ...
    ### print(len(ydata_whole), ydata_whole)
    ...
    """
    print('Pay attention', int(m_original), m_original)
    if int(m_original) != 0 or int(m_original) == 0:
        print(len(delta_ydata_new), delta_ydata_new)
        print(len(ydata), ydata)
        print(len(delta_ydata), delta_ydata)
        print(initial_year_index)
    """
    #
    ydata_new = [0 for i in range(len(ydata_whole))]
    # ydata_new[0] = ydata_whole[0]
    ...
    for i in range(len( delta_ydata )):
        # print('\n')
        # print(i)
        # print(time_list[i+initial_year_index], i, initial_year_index)
        if time_list[i+initial_year_index] <= finyear:
            # print('all good')
            apply_delta_ydata_new = delta_ydata_new[i]
        else:
                ...
                if str( scenario_list[s] ) in Scenarios_Involved:
                    ...
                    for p in range( len( Parameters_Involved ) ):
                        ...
                        #
                            ...
                            #
                        #
                        elif X_Cat in ['Agricultural Demand', 'Agricultural Yield', 'Agricultural Yield Livestock']:
                            #------------------------------------------
                            for a_set in range( len( Sets_Involved ) ):
                                ...
                                elif 'Agricultural Yield Livestock' == X_Cat:
                                ...
                                for n_afuel in range(this_set_fuels_len):
                                    ...
                                    # if 'Agricultural Yield' == X_Cat and this_set == 'Cafe':
                                    #    print("review behaviour 1")
                                    #    sys.exit()

                                    # if 'Agricultural Yield' == X_Cat and this_set == 'Cana':
                        ...
                        #
                            ...
                            # print('Work on the forest cover bit.')
                            # sys.exit()
